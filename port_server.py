...
def communicate(client, addr, data):
    ...
    if data.split(':')[0] == 'add' and data.count(':') == 3: add_connection(client, data.split(':')[1:])
    if data.split(':')[0] == 'get' and data.count(':') == 1: ask_for_port(client, data.split(':')[1:])
    if data.split(':')[0] == 'del' and data.count(':') == 1: delete_query(client, data.split(':')[1:])
...
def main():
    global is_waiting, is_running, read_sockets, write_sockets, addresses
    ...
    while is_running:
        ...
        for socket in readable:
            if socket == server_socket:
                ...
                try: data = socket.recv(BUFFER).decode()
                except s.error: continue
                communicate(socket, addresses[socket], data)
